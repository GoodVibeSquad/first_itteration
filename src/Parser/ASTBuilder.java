            ...
            case "functionIdentifier" -> {
                if (children.size() == 5) {  // Function af denne form : TYPE ID ( expr_list )
                    ...
                    if (type instanceof Token typeToken && id instanceof Token idToken && exprList instanceof EList params) {
                        // Brug to-strengs constructor for at sætte både id og targetType rigtigt
                        Identifier name = new Identifier(idToken.getValue());
                        Identifier var = new Identifier(idToken.getValue(), typeToken.getValue()); // her er targetType sat korrekt
                    ...
                    if (type instanceof Token typeToken && id instanceof Token idToken) {
                        Identifier name = new Identifier(idToken.getValue());
                        Identifier var = new Identifier(idToken.getValue(), typeToken.getValue()); // targetType gemmes
...
//                else if(children.get(1) instanceof Type targetType && targetType.tok == TokenType.TYPE && children.size() == 5){
//
...
//                }
                    ...
                    try {
                        ...
                        System.err.println("Unknown targetType: " + typeName);
                        throw new RuntimeException("Unknown targetType: " + typeName);
                    }
                }
